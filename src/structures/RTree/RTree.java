...
public class RTree {
    private final double SMOOTH_ADDITION = 0.001;
    ...
    public MyArrayList cloneMyArrayList(MyArrayList list){

        MyArrayList<NodeR> aux = new MyArrayList<>(list.getSize());

        for(int i = 0; i < list.getSize(); i++)
            aux.add((NodeR) list.get(i));
    ...
    public NodeR partition(NodeR n){
        ...
        MyArrayList<NodeR> aux = cloneMyArrayList(n.getNodes());

        //Remove elements from actual node
        n.getNodes().removeAll(n.getNodes());
        ...
        coords[1] = average[1] + SMOOTH_ADDITION;
        coords[2] = maxX;
        coords[3] = maxY;
        n.getNodes().add(new NodeR(coords, null));
